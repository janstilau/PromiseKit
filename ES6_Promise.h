...
//  PromiseKit
//
//  Created by JustinLau on 2022/8/20.
//

#ifndef ES6_Promise_h
#define ES6_Promise_h
...
/*
 ...
 // 回调函数, 可以理解为传递 Block 对象, 也可以理解为使用接口编程, 就是 Delegate 这种方式.
 // 事件, 可以使用 TargetAction, 和 Notification 来进行理解.
 // 虽然从使用场景下, 稍显不同, 但是内部实现都是存储了一个对象, 然后存储了一个对象中的回调函数的信息. 无论是直接存储函数指针, 还是存储了所触发函数的元信息, 都是将回调进行了管理.
 // 不同的是, 暴露给使用者的感官概念不同. TargetAction, Notification 这两种方式, 给使用者的感觉就是事件进行了触发, 而 Block, 或者 Delegate 这种方式, 则是使用接口进行编程.
 ...
 Promise 里面, 存储了异步结果. 以及这个异步结果在获取之后, 应该触发什么样的操作.
 当异步结果确定了之后, 会触发所有存储的后续操作. 一般来说, 这个后续操作, 会返回一个新的 Promise 对象.
 这个过程中, 有个比较费解的地方. 在注册事件的时候, 返回一个 Promise 对象, 在后续操作里面, 也是返回一个 Promise 对象, 这两个对象不是一个对象.
 注册 Promise 的值, 是由操作 Promise 的值确定下来的.

 `Promise`对象有以下两个特点。

 // 从这里来看, set 函数一定会有线程控制.
 // 并且 set 函数是一个一次性的函数.
 （1）对象的状态不受外界影响。`Promise`对象代表一个异步操作，有三种状态：`pending`（进行中）、`fulfilled`（已成功）和`rejected`（已失败）。
    只有异步操作的结果，可以决定当前是哪一种状态，任何其他操作都无法改变这个状态。这也是`Promise`这个名字的由来，它的英语意思就是“承诺”，表示其他手段无法改变。

 // 从这里来看, get 函数里面, 一定也有着线程的控制 .
 （2）一旦状态改变，就不会再变，任何时候都可以得到这个结果。`Promise`对象的状态改变，只有两种可能：从`pending`变为`fulfilled`和从`pending`变为`rejected`。只要这两种情况发生，状态就凝固了，不会再变了，会一直保持这个结果，这时就称为 resolved（已定型）。如果改变已经发生了，你再对`Promise`对象添加回调函数，也会立即得到这个结果。这与事件（Event）完全不同，事件的特点是，如果你错过了它，再去监听，是得不到结果的。

 注意，为了行文方便，本章后面的`resolved`统一只指`fulfilled`状态，不包含`rejected`状态。

 // 层层嵌套这个现象, 从代码调用的角度来说, 是无法避免的. 因为就是在异步操作之后, 才应该触发后续的操作.
 // 而 Promise 这种使用同步流程来表达异步操作的写法, 其实是 Promise 内部增加了一个机制, 将异步操作的回调, 使用更加友好的方式将回调进行了管理.
 // 异步操作触发的顺序, 还是按照之前的顺序. 触发的时机, 还是上一个异步完成之后, 触发下一个异步操作.
 ...
 // 这是一个通用的设计行为, 应该说, 其他语言, 平台都是使用了这样的方式, 进行的 Promise 的设计.
 const promise = new Promise(function(resolve, reject) {
   // ... some code
    // 这里是伪代码, 真正的异步操作的触发时机, 是在 状态确定的那一刻.
   if (/* 异步操作成功 */){
     resolve(value);
   } else {
     reject(error);
...
// 里面一定有着锁的控制.
 ...
 // 这里是关键所在, 为 Promise 增加了状态封箱后的操作.
 // 并且这个函数, 返回了一个新的 Promise 对象. 可以继续向返回的 Promise 对象里面, 使用 then 添加操作.
 // 这里就会触发上面提到的, 两个 Promise 联动的效果.
 ...
 ```javascript
 function timeout(ms) {
     // Promise 对象初始化的时候, 传递过去的是异步操作的启动函数.
     // 这个启动函数的参数, 是 resolve, reject, 也就是对于这个 Promise 对象的封箱操作.
     // 在这函数的异步启动操作中, 是设置一个定时器, 然后在定时器到达之后, 触发封箱操作.
...
// 这里就是再说两个 Promise 的联动.
 上面代码中，`timeout`方法返回一个`Promise`实例，表示一段时间以后才会发生的结果。过了指定的时间（`ms`参数）以后，`Promise`实例的状态变为`resolved`，就会触发`then`方法绑定的回调函数。

 Promise 新建后就会立即执行。
 ```javascript
// Promise 的启动函数里面, 没有触发任何的异步操作. 直接进行了封箱函数的调用.
// 所以, Promise 在创建之后, 里面的状态就固定下来了.
 let promise = new Promise(function(resolve, reject) {
   console.log('Promise');
   resolve();
 });

// 这个时候的 then 操作, 面对的是已经处于封箱状态的 Promise. 但是 Then 操作中添加的操作, 还是异步的. 在当前运行循环之后才触发的.
 ...
 function loadImageAsync(url) {
     // Promise 的构造函数里面, 是触发了一个异步操作. 在这个异步操作的最后, 来完成了 Promise 的封箱处理.
...
// 这里所说的, 就是两个 Promise 联动的问题.
 上面代码中，`p1`和`p2`都是 Promise 的实例，但是`p2`的`resolve`方法将`p1`作为参数，即一个异步操作的结果是返回另一个异步操作。
 注意，这时`p1`的状态就会传递给`p2`，也就是说，`p1`的状态决定了`p2`的状态。如果`p1`的状态是`pending`，那么`p2`的回调函数就会等待`p1`的状态改变；如果`p1`的状态已经是`resolved`或者`rejected`，那么`p2`的回调函数将会立刻执行。

 ```javascript
// 启动了一个定时器, 这个定时器会在 3 秒钟之后, 进行封箱的操作.
 const p1 = new Promise(function (resolve, reject) {
   setTimeout(() => reject(new Error('fail')), 3000)
 })

// 启动了一个定时器, 这个定时器, 会在 1 秒钟之后, 进行封箱的操作.
// 但是封箱得到的值是一个 Promise, 那么就等待这个 Promise, 这个 Promise 的值作为封箱值.
 const p2 = new Promise(function (resolve, reject) {
...
// 这里是代码写法的建议, 就是在封箱操作之后, 不要进行任何的其他多余操作.
