...
public extension Thenable {
    ...
    func flatMap<U>(on: DispatchQueue? = shareConf.defaultQueue.map, _ transform: @escaping(T) throws -> U?) -> Promise<U> {
...
public extension Thenable where T: Sequence {
    ...
    func map<U>(on: DispatchQueue? = shareConf.defaultQueue.map, _ transform: @escaping(T.Iterator.Element) throws -> U) -> Promise<[U]> {
        return mapValues(on: on, transform)
    }

    /// disabled due to ambiguity with the other `.flatMap`
    @available(*, deprecated, message: "See: `flatMapValues`")
    func flatMap<U: Sequence>(on: DispatchQueue? = shareConf.defaultQueue.map, _ transform: @escaping(T.Iterator.Element) throws -> U) -> Promise<[U.Iterator.Element]> {
        return flatMapValues(on: on, transform)
    }
#endif

    @available(*, deprecated, message: "See: `filterValues`")
    func filter(on: DispatchQueue? = shareConf.defaultQueue.map, test: @escaping (T.Iterator.Element) -> Bool) -> Promise<[T.Iterator.Element]> {
...
public extension Thenable where T: Sequence, T.Iterator.Element: Comparable {
    ...
    func sorted(on: DispatchQueue? = shareConf.defaultQueue.map) -> Promise<[T.Iterator.Element]> {
