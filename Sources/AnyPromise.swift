...
public class __AnyPromise: NSObject {
    ...
    @objc public init(resolver body: (@escaping (Any?) -> Void) -> Void) {
        ...
        body {
            if let p = $0 as? AnyPromise {
                p.getInternalPromise.__pipe(self.box.seal)
            } else {
                self.box.seal($0)
            }
        }
    ...
    @objc public func __thenOn(_ q: DispatchQueue,
                               execute: @escaping (Any?) -> Any?) -> AnyPromise {
...
extension AnyPromise: Thenable, CatchMixin {
    ...
    fileprivate var getInternalPromise: __AnyPromise {
        return value(forKey: "__d") as! __AnyPromise
    }
    
    var box: Box<Any?> {
        return getInternalPromise.box
    }
